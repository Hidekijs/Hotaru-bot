const dataBase = async(sock, m, db) => {
	try{
		...
		let welDefault = `@user sea bienvenido a "@group"\n\n*No olvides leer las reglas:*\n\n@desc`;
		let byeDefault = `Adios @user`;
		...
		if (m.isGroup && (m.from.endsWith('@g.us'))) {
			...
			if (chat) {
				...
				if (!('customWel' in chat)) chat.customWel = welDefault;
				if (!('customBye' in chat)) chat.customBye = byeDefault;
